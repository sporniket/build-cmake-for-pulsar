...
import * as configForPulsar from './config-for-pulsar.js'
...
const CONFIGURATION_KEYS = ['executable','buildRoot','cmakeCommandArgs','buildCommandArgs']
...
export function provideCMake() {
    return class CMakeBuildProvider extends EventEmitter {
        constructor(projectDirectory) {
            ...
            this.settingsCache = configForPulsar.loadAndWatchSettingsFromPulsar(
                atom,
                CONFIGURATION_KEYS,
            ...
            this.cmake = new CMake(this.settingsCache.get('executable').get('value'));
        ...
        destructor() {
            ...
            this.settingsCache.forEach((value, key, map) => {
                value.get('subscription').dispose();
                value.set('subscription', undefined);
            })
        ...
        settingsDidChange(key, newValue, oldValue, map) {
          if(key === map.get('executable').get('key')) {
            this.renewCmake();
          }
          this.refresh();
        }
