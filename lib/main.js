...
import * as configHelper from 'config-helper-for-pulsar-by-sporniket'
...
function isOfInterest(pathOfFile) {
    return FILENAMES_OF_INTEREST.some(e => pathOfFile.endsWith(e));
}
...
export function provideCMake() {
    return class CMakeBuildProvider extends EventEmitter {
        constructor(projectDirectory) {
            ...
            this.settingsCache = configHelper.loadAndWatchSettingsFromPulsar(
                atom,
                'build-cmake-for-pulsar-by-sporniket',
                {
                    executable: this.renewCmake,
                    buildRoot: this.refresh,
                    cmakeCommandArgs: this.refresh,
                    buildCommandArgs: this.refresh
                }
            )

            this.cmake = new CMake(this.settingsCache.executable.value);
        ...
        destructor() {
            ...
            configHelper.dispose(this.settingsCache);
        ...
        renewCmake() {
            try {
                const newCmake = new CMake(this.settingsCache.executable.value);
                this.cmake = newCmake;
                this.refresh();
            } catch(e) {
                // whatever, surely wrong path
            }
        }
                    ...
                    if (generator) {
                        return ['Debug', 'Release', 'RelWithDebInfo', 'MinSizeRel'].map(configurationType => {
                            return {
                                ...
                                buildRoot: this.settingsCache.buildRoot.value,
                                cmakeCommandArgs: this.settingsCache.cmakeCommandArgs.value,
                                buildCommandArgs: this.settingsCache.buildCommandArgs.value
                            };
                        });
                    } else {
                        return [];
