  ...
  return class CMakeBuildProvider extends EventEmitter {
    constructor(source_dir) {
      ...
      atom.config.observe('build-cmake.cmakelists', (cmakelists) => {
        this.source_dir =
            (!!cmakelists) ? source_dir + cmakelists.trim() : source_dir;
      });
      atom.config.observe('build-cmake.build_dir', (buildDir) => {
        ...
        const pathVars = {
          ...
          PROJECT_DIRNAME: path.basename(this.project_dir)
        };
      ...
      atom.config.onDidChange('build-cmake.build_dir', () => {
        this.emit('refresh');
      });
      ...
      atom.config.onDidChange('build-cmake.cmake_arguments', () => {
        this.emit('refresh');
      });
      atom.config.onDidChange('build-cmake.build_arguments', () => {
        this.emit('refresh');
      });
      atom.config.onDidChange('build-cmake.parallel_build', () => {
        this.emit('refresh');
      });
    }
    ...
    setAvailableGenerators(generators) {
      ...
      atom.config.setSchema(
          ...
          Object.assign(
              defaultConfig.generator, {enum: [''].concat(generators)}));
    ...
    validateExecutable() {
      ...
      function extractGenerators(stdout) {
        ...
        for (let i = 0; generatorsSplit.length > i; i += 2) {
          let generatorStr = generatorsSplit[i] + generatorsSplit[i + 1];
    ...
    visualStudioTargets() {
      return Array
          .from(new Set(glob.sync('**/*.vcxproj', {
                              cwd: this.build_dir,
                              nodir: true,
                              ignore: 'CMakeFiles/**'
                            })
    ...
    createMakeFileTarget(target_name) {
      args_list = ['--build', this.build_dir, '--target', target_name, '--'];
      if (this.parallel_build) args_list.push('-j' + os.cpus().length);
      return {
        atomCommandName: 'cmake:' + target_name,
        name: target_name,
        exec: this.executable,
    ...
    makeFileTargets() {
      output = execSync(
          this.executable + ' --build "' + this.build_dir + '" --target help',
          {cwd: this.build_dir});
      return output.toString('utf8')
          .split(/[\r\n]{1,2}/)
          .filter(line => line.startsWith('...'))
          .map(
              (line) => this.createMakeFileTarget(
    ...
    createNinjaTarget(target_name) {
      return {
        ...
        exec: this.executable,
        cwd: this.build_dir,
        args: ['--build', this.build_dir, '--target', target_name].concat(
            this.build_arguments),
        errorMatch: compileErrorMatch.concat(generateErrorMatch),
        warningMatch: compileWarningMatch.concat(generateWarningMatch),
        sh: false
      };
    ...
    ninjaTargets() {
      output = execSync(
          this.executable + ' --build "' + this.build_dir + '" -- -t query all',
          {cwd: this.build_dir});
      ...
      output = execSync(
          this.executable + ' --build "' + this.build_dir + '" -- -t targets',
          {cwd: this.build_dir});
      targets =
    ...
    settings() {
      ...
      var args = this.cmake_arguments.concat([
        '-B' + this.build_dir, '-H' + this.source_dir,
